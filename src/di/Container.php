...
class Container
{
    ...
    public function get($class, $params = array(), $properties = array())
    {
        ...
        if (is_callable($definition, true)) {//如果声明是函数
            ...
            //definition中除了'class'元素外 其他的都当做实例的属性处理
        ...
        } elseif (is_object($definition)) {//如果是个对象 直接返回
            return $definition;
        } else {
            throw new ArgumentException($class . ' 声明错误!');
        }
        return $obj;
    }
        ...
        foreach ((array)$dependencies as $index => $dependency) {
            if ($dependency instanceof Instance) {
                if ($dependency->id !== null) {
                    ...
                    throw new ArgumentException('实例化类 ' . $class . ' 时缺少必要参数:' . $name);
                }
            }
        }
        return $dependencies;
